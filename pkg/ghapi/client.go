...
// NewGitHubClient creates a new GitHub client using the same authentication and HTTP client
// setup as gh CLI, but configured to work with go-github/v74/github
func NewGitHubClient(hostname, token, version string) (*github.Client, error) {
	...
	// Create GitHub client with the configured HTTP client
	client := github.NewClient(httpClient)

	// Set base URL based on hostname (similar to gh CLI's approach)
	if hostname != "" && hostname != "github.com" {
		// For GitHub Enterprise
		client.BaseURL.Scheme = "https"
		client.BaseURL.Host = hostname
